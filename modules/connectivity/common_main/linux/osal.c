...
/* 4.11 sched.h header split: local_clock() and show_stack() now live in
 * dedicated headers (API-CHURN-LOG.md M4/M5; frank-w forum catalog). */
...
{
	/* show_stack() is not exported to modules on 6.6;
	 * sched_show_task() (EXPORT_SYMBOL_GPL) prints the same
	 * task-state + stack for debugging (API-CHURN-LOG.md M4). */
...
/* v4.15 removed init_timer()/timer_list.data; timer_setup() handlers take
 * the timer_list pointer instead. Trampoline recovers the OSAL_TIMER and
 * calls the vendor handler with its original ULONG argument, so every OSAL
 * caller keeps the 3.18 semantics. (API-CHURN-LOG.md T1; frank-w hit the
 * same conversion in the 4.14->4.19 era.) */
