...
static bool reset;  /* churn: module_param(bool) needs a bool */
...
static ssize_t file_read(char *filename, char *buf, size_t len, loff_t *offset)
{
    ...
    /* churn: set_fs()/f_op->read removed; kernel_read handles it */
    retLen = kernel_read(fp, buf, len, offset);
...
static ssize_t file_write(char *filename, char *buf, size_t len, loff_t *offset)
{
    ...
    /* churn: set_fs()/f_op->write removed; kernel_write handles it */
    retLen = kernel_write(fp, buf, len, offset);
...
void hci_stp_dev_init_rx_cb (const PUINT8 data, INT32 count) /* churn: match MTK_WCN_STP_IF_RX */
{
    struct hci_stp *hu;
    unsigned int idx;
...
#else
    hdev->bus = HCI_UART;
    hdev->dev_type = HCI_PRIMARY; /* churn: HCI_BREDR renamed (v4.8) */
#endif
    hci_set_drvdata(hdev, hu);

    BT_DBG_FUNC("hci_get_drvdata(hdev)\n");
...
static void hci_stp_exit(void)
{
    struct hci_stp *hu = (struct hci_stp *)hci_get_drvdata(hdev);
