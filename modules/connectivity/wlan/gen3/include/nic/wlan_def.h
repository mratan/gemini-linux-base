...
/* The kernel's <acpi/actypes.h> (pulled in transitively on 6.6 via the
 * device.h include chain, unlike 3.18) defines ACPI_STATE_D0..D3 as macros,
 * which collides with this driver-private enum. The driver uses only its own
 * values and never the kernel ACPI states, so drop the macros here.
 * API-CHURN-LOG.md G2 (novel — 3.18 never saw the acpi header). */
#undef ACPI_STATE_D0
#undef ACPI_STATE_D1
#undef ACPI_STATE_D2
#undef ACPI_STATE_D3
