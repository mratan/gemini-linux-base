...
/* v5.8: .mgmt_frame_register(frame_type,reg) -> .update_mgmt_frame_registrations(upd)
 * (API-CHURN-LOG.md G13). */
void mtk_p2p_cfg80211_update_mgmt_frame_registrations(IN struct wiphy *wiphy,
						      struct wireless_dev *wdev,
						      struct mgmt_frame_regs *upd)
{
	P_GLUE_INFO_T prGlueInfo = (P_GLUE_INFO_T) NULL;
	bool fgProbeReq, fgAction;

	if (wiphy == NULL)
		return;

	DBGLOG(P2P, TRACE, "mtk_p2p_cfg80211_update_mgmt_frame_registrations\n");

	prGlueInfo = *((P_GLUE_INFO_T *) wiphy_priv(wiphy));

	fgProbeReq = !!(upd->interface_stypes & BIT(IEEE80211_STYPE_PROBE_REQ >> 4));
	fgAction = !!(upd->interface_stypes & BIT(IEEE80211_STYPE_ACTION >> 4));
