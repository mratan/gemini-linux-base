...
#include "gl_wext.h"
#include "precomp.h"
/* struct inet6_dev / inet6_ifaddr are opaque without these on 6.6; the
 * boottime ts64 API and sched_show_task also live here. 3.18 got them
 * transitively. (API-CHURN-LOG.md G7) */
...
WLAN_STATUS kalFirmwareOpen(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	/* set_fs()/get_fs()/KERNEL_DS removed in v5.10; kernel_read() (used
	 * below) needs no address-limit override (API-CHURN-LOG.md G4). */
...
WLAN_STATUS kalRxIndicateOnePkt(IN P_GLUE_INFO_T prGlueInfo, IN PVOID pvPkt)
{
	...
	/* net_device.last_rx removed in v4.11 (API-CHURN-LOG.md G6) */
#if CFG_SUPPORT_SNIFFER
	if (prGlueInfo->fgIsEnableMon) {
	...
	case WLAN_STATUS_MEDIA_CONNECT:
		...
		if (prGlueInfo->fgIsRegistered == TRUE) {
			...
			if (eStatus == WLAN_STATUS_ROAM_OUT_FIND_BEST) {
				/* cfg80211_roamed_bss() was folded into
				 * cfg80211_roamed() taking a cfg80211_roam_info
				 * in v4.12; the bss/ie fields moved under
				 * links[0] in the v6.0 MLO rework
				 * (API-CHURN-LOG.md G11). */
...
static void kalTimerTrampoline(struct timer_list *t)
{
...
VOID kalOsTimerInitialize(IN P_GLUE_INFO_T prGlueInfo, IN PVOID prTimerHandler)
{
	...
	/* timer_setup() replaced init_timer()+.data in v4.15
	 * (API-CHURN-LOG.md G5). The trampoline recovers prGlueInfo from the
	 * embedded tickfn and calls kalTimeoutHandler with it, preserving the
	 * (unsigned long)prGlueInfo argument the vendor handler expects. Only
	 * kalTimeoutHandler is ever passed here (gl_init.c). */
	(void)prTimerHandler;
