...
/* The private-ioctl (iwpriv) members of struct iw_handler_def only exist
 * under CONFIG_WEXT_PRIV on 6.6 (a select-only symbol, off in our config).
 * Guard them so the driver builds either way; when WEXT_PRIV is enabled the
 * full iwpriv interface is wired as before. API-CHURN-LOG.md G8 (semantic:
 * iwpriv commands are unavailable in the default build). */
