...
#include "precomp.h"
/* struct sched_param moved to uapi sched/types.h; show_stack in sched/debug.h (API-CHURN-LOG.md G10) */
...
/* ndo_select_queue lost accel_priv (v4.19) and fallback (v5.2); now (dev, skb, sb_dev) (API-CHURN-LOG.md G15) */
...
static void createWirelessDevice(void)
{
	...
	/* WIPHY_FLAG_SUPPORTS_SCHED_SCAN removed v3.15; max_sched_scan_reqs>0 advertises it (API-CHURN-LOG.md G14) */
...
bailout:
		...
		/* v5.9 unexported sched_setscheduler() to modules; the sanctioned
		 * replacement is sched_set_fifo() (SCHED_FIFO at a low RT prio).
		 * The vendor's exact numeric prio/policy is not reproducible via
		 * the exported helpers, so all three Wi-Fi kthreads get FIFO-low.
		 * API-CHURN-LOG.md G19 (semantic: exact prio value not preserved). */
...
#endif
			DBGLOG(INIT, INFO,
			       "Set pri = %d, sched = %d (mapped to SCHED_FIFO low)\n",
		...
		/* set MAC address */
		{
			...
			kalMemZero(&MacAddr, sizeof(MacAddr));
			rStatus = kalIoctl(prGlueInfo,
...
/* MODULE_SUPPORTED_DEVICE removed in v5.12 (API-CHURN-LOG.md G9) */
MODULE_LICENSE("GPL");
