...
/* v5.8 replaced the per-frame-type .mgmt_frame_register(frame_type, reg)
 * op with .update_mgmt_frame_registrations(upd), where upd->interface_stypes
 * is a bitmask indexed by (frame subtype >> 4). We translate the two frame
 * types this driver cares about (PROBE_REQ, ACTION) back into the original
 * per-type filter logic. API-CHURN-LOG.md G13 (semantic). */
						  ...
						  IN struct mgmt_frame_regs *upd)
{
	...
	fgProbeReq = !!(upd->interface_stypes &
			BIT(IEEE80211_STYPE_PROBE_REQ >> 4));
	fgAction = !!(upd->interface_stypes &
...
/* fgIsTDLSlinkEnable = 0; */
	...
	/* v4.4 folded the mac arg into struct station_del_parameters
	 * (API-CHURN-LOG.md G12). A NULL params->mac means "all stations". */
